package com.jbrown.web.ws.responder;

public interface ResponderK {
	String AUTH_RESPONDER = "authResponder";
	String REGISTER_RESPONDER = "registerResponder";
	String LOGIN_RESPONDER = "loginResponder";
	String LOGOUT_RESPONDER = "logoutResponder";
	String GEO_RESPONDER = "geoResponder";
	String UTIL_RESPONDER = "utilityResponder";
	String MEDIA_RESPONDER = "mediaResponder";
	String AIRPORT_RESPONDER = "airportResponder";
	String AIRPORT_IATA_RESPONDER = "airportIATAResponder";
	String CACHE_PUSH_RESPONDER = "cachePushResponder";
	String CACHE_GET_RESPONDER = "cacheGetResponder";
	String QUIZ_ALL_QUESTIONS_RESPONDER = "quizAllQuestionsResponder";
	String INAVLID_REQUEST_RESPONDER = "invalidRequestResponder";
}
