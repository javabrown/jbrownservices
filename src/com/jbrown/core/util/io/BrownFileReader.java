...
public class BrownFileReader {
  ...
  private static Logger logger =
    Logger.getLogger(BrownFileReader.class.getName());
  
  public BrownFileReader(String name, String fileName, String separatorRE,
			FieldsProcessorI fieldsProcessor) 
  {
    _name = name; _in = getFileReader(fileName); 
    _separatorJB = separatorRE; _fieldsProcessor = fieldsProcessor;
  }
  
  public BrownFileReader(String name, InputStream in, String separatorRE,
			FieldsProcessorI fieldsProcessor) 
  {
    _name = name; _in = new BufferedReader(new InputStreamReader(in)); 
    _separatorJB = separatorRE; _fieldsProcessor = fieldsProcessor;
  }
  ...
  public void read() {
    try {
      ...
      lineN++;
      for (String line = _in.readLine(); line != null; line = _in.readLine()) {
        String[] fields = line.split(_separatorJB, -1);
        String status = 
          _fieldsProcessor.processFields(fields);
        if (status != null) {
          logger.error(_name + ":" + lineN + ": " + status);
        }
        lineN++;
      } //for
      logger.info(_name + ": read " + (lineN-1) + " lines from " + _name);
    } //try
    catch (IOException e) {
      throw new RuntimeException(e);
    }
